...
export default function Dashboard() {
  ...
  const addShed = () => {
    if (!shedName.trim()) {
      toast({ title: "Error", description: "Enter shed name", variant: "destructive" });
      return;
    }
    const newShed = { id: Date.now(), name: shedName, looms: [] };
    setSheds([...sheds, newShed]);
    setShedName("");
    toast({ title: "Success", description: "Shed added successfully" });
  };

  const addLoom = () => {
    if (!selectedShed || !loomNum.trim()) {
      toast({ title: "Error", description: "Select shed and enter loom number", variant: "destructive" });
      return;
    }
    setSheds(sheds.map(shed => {
      if (shed.id === parseInt(selectedShed)) {
        return {
          ...shed,
          looms: [...shed.looms, { id: Date.now(), loom_number: loomNum }]
        };
      }
      return shed;
    }));
    setLoomNum("");
    toast({ title: "Success", description: "Loom added to shed" });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card-elevated p-6 animate-slide-up" style={{ animationDelay: "0.2s" }}>
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="font-bold text-lg text-foreground">Loom Config</h2>
          </div>
        ...
        <div className="card-elevated p-6 flex flex-col justify-center animate-slide-up" style={{ animationDelay: "0.3s" }}>
          <div className="flex items-center gap-3 mb-5">
